# mpc_robustness_funcs_more_controls.R
...
ivregression <- function(df,controls) {
  ...
  reg <- function(y, x, df,controls) {
    #generic IV regression of y vs x; instrument post_treat_ix is always the same; cluster SEs by state
    
    df <- df %>%
      left_join(cust_covariates_binned,by="cust_number")
    if (controls == "none"){reg_result <-
      felm(as.formula(str_c(y, " ~ post + treat  |0| (", x, " ~ post_treat_ix) | cust_number")),
           data = df,
           weights = df$weight,
           psdef = FALSE)}
    else if (controls == "state"){reg_result <-
          felm(as.formula(str_c(y, " ~ post + treat + state_factor * post |0| (", x, " ~ post_treat_ix) | cust_number")),
           data = df,
           weights = df$weight,
           psdef = FALSE)}
    else if (controls == "state_age"){reg_result <-
          felm(as.formula(str_c(y, " ~ post + treat + state_factor * post + age_bin * post |0| (", x, " ~ post_treat_ix) | cust_number")),
           data = df,
           weights = df$weight,
           psdef = FALSE)}
    else if (controls == "state_age_kids"){reg_result <-
      felm(as.formula(str_c(y, " ~ post + treat + state_factor * post + age_bin * post + has_kids_factor * post |0| (", x, " ~ post_treat_ix) | cust_number")),
           data = df,
           weights = df$weight,
           psdef = FALSE)}
    else if (controls == "state_age_kids_industry"){reg_result <-
      felm(as.formula(str_c(y, " ~ post + treat + state_factor * post + age_bin * post + has_kids_factor * post + ind_factor * post |0| (", x, " ~ post_treat_ix) | cust_number")),
           data = df,
           weights = df$weight,
           psdef = FALSE)}
    else if (controls == "age_kids"){reg_result <-
      felm(as.formula(str_c(y, " ~ post + treat + age_bin * post + has_kids_factor * post |0| (", x, " ~ post_treat_ix) | cust_number")),
           data = df,
           weights = df$weight,
           psdef = FALSE)}
  ...
  # These regressor names help extract the estimates and clustered se for tbl_out
  regressor_names <- rep(c("`income(fit)`", 
                           "`total_ui_inflows(fit)`"), 2)
  tbl_out <- data.frame(
    numerator = c(rep('Spend (total)', 2), 
                  rep('Spend (cardcash)', 2)),
    denominator = rep(c('income', 'ui'), 2),
    apc = (unlist(map2(seq_along(iv_list), 
                       regressor_names, 
                       ~ iv_list[[.x]][[1]][.y, "Estimate"])) %>% 
             round(3)),
    se = (unlist(map2(seq_along(iv_list), 
                      regressor_names, 
                      ~ iv_list[[.x]][[1]][.y, "Cluster s.e."])) %>% 
            round(3)),
...
# basic structure: follow line 204:
# map2(iv_list, c(1:4), ~.x[[.y]][[1]]["`income(fit)`", "Estimate"])

#This function merges in the relevant re-weights defined in spend_by_liquidity_buffer and spend_by_ever_recall
#when weights are multiplied by re_weights, the new weights will match income and EIP week not just for treatment and control (E and U) but also
#across the observable splits like high and low liquidity, so that we will be comparing low liquidity and high liquidity households with the same income
#This is re_weight is a "wrapper" outside the weighting of treatment and control, so reweights both the U and E groups accordingly, i.e.
#everywhere below now uses weight=weight*re_weight but re_weight will be set equal to 1 except in the cases of recall and liquidity where we want reweighting
#browser()
join_weights_with_re_weights <- function(df_weights,sample) {
  if (sample == "not_recalled" | sample =="to_payroll") {
    df_weights <- df_weights %>% left_join(weights_start_no_recall,by=c("inc_2019_quintile_index", "eip_week")) %>%
      replace_na(list(re_weight = 0))
  } else if (sample == "high_liquidity" | sample =="low_liquidity") {
    df_weights <- df_weights %>% left_join(reweights_to_use_for_liquidity,by=c("inc_2019_quintile_index", "eip_week")) %>%
      replace_na(list(re_weight = 0))
    if (sample == "high_liquidity") { #use reweight where income of high liquidity group matches overall pop
      df_weights<-df_weights %>% mutate(re_weight=re_weight_high)
    } else { #use reweight where income of low liquidity group matches overall pop
...
run_apc_calculations <- function(df_list, sample, controls) {
  ...
  customer_input_dfs <- c((list(cust_thru_aug, cust_thru_jan,cust_ui_thru_sept_2021,cust_ui_thru_jul_2021) %>%
  ...
  weights_waiting_apc <- join_weights_with_re_weights(weights_waiting_apc,sample)
  weights_lwa_apc <- join_weights_with_re_weights(weights_lwa_apc,sample)
  weights_expiration_apc <- join_weights_with_re_weights(weights_expiration_apc,sample)
  weights_onset_apc <- join_weights_with_re_weights(weights_onset_apc,sample)
  weights_puc_sep_apc <- join_weights_with_re_weights(weights_puc_sep_apc,sample)
  weights_puc_jun_apc <- join_weights_with_re_weights(weights_puc_jun_apc,sample)
  weights_puc_jun_vs_sep_apc <- join_weights_with_re_weights(weights_puc_jun_vs_sep_apc,sample)
  ...
  periodid_filters <- list(c(202003, 202005),
                           ...
                           c(202106, 202108),
                           c(202106, 202108))
  innerjoin_dfs <- list(waiting_apc_customers, ui_onset_customers, ui_expiration_customers,ui_puc_sep_customers,ui_puc_jun_customers,ui_puc_customers_only_u)
  ...
  if(sample == "all" | sample=="recalled" | sample=="not_recalled" | sample=="to_payroll" | sample=="low_liquidity" | sample=="high_liquidity"){ 
    ...
    iv_reg_all <- map(seq_along(dfs_iv), 
                      ~dfs_iv[[.x]] %>%
                        ...
                        ivregression(controls) %>% mutate(design = iv_design[.x])) %>%
    ...
    if(sample == "all"){
      table_iv_apcs_combined %>% write_csv(str_c(path_out,
                                                 'apcs_table_ivreg_controls_',
                                                 controls,
                                                 '.csv')) #APC results from IV regressions
    }
    if(sample == "recalled"){
      table_iv_apcs_combined %>% write_csv(str_c(path_out, 
